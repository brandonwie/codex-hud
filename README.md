...
The installer first looks for a target-specific archive under the `codex-runtime-v<version>` release. It verifies the adjacent SHA-256 file, requires a manifest containing the current patch-set revision, upstream source commit, and payload SHA-256, requires the upstream `LICENSE` and `NOTICE`, and then runs the patched binary's `--version` health check. A stale same-version prebuilt therefore fails closed instead of being mistaken for the current patch. If no matching release asset exists, non-Intel targets retain the source-build fallback. Intel macOS fails fast instead of unexpectedly starting an hour-scale link; a maintainer publishes the missing asset either by dispatching the **Patched Codex Runtime** workflow with its `target` input (`aarch64-apple-darwin` on the standard Apple Silicon runner, `x86_64-apple-darwin` on the Intel runner behind the `local_build_ok` runner-minute guard) or from a local machine — a `--source-build` compile, then `npm run package:runtime -- --payload ~/.local/bin/codex-hud-codex.d/<version>/codex --version <version> --target <triple> --source ~/.cache/codex-hud/openai-codex-rust-v<version>` and `gh release upload codex-runtime-v<version> dist/*` (both paths use the same packager, so the bundles are byte-compatible) — or pass `--source-build` to compile locally with LTO disabled and 16 release codegen units. In both cases, the executable is staged under `~/.local/bin/codex-hud-codex.d/<version>/codex` **before** anything is activated; only then is `~/.local/bin/codex-hud-codex` atomically retargeted to the new payload, and the previous version is kept on disk for rollback. A failed payload is kept aside as `<version>.failed` and the active runtime is left untouched. The installer also writes `~/.local/bin/codex-hud-tui` in patched mode, a launcher that passes the colored status-line command through Codex's `-c tui.status_line_command=...` override without changing `~/.codex/config.toml`. With the default `--renderer auto`, the injected command is `'~/.local/bin/codex-hud' --line --color`; if that Rust renderer is missing or fails its health check, the patched install stops instead of falling back to another renderer. The executable path and `argv[0]` both keep Codex-visible names, so terminal integrations such as Herdr can still recognize the pane as a Codex session.
