...
| Experimental patched TUI footer | Not published (retired 2026-09-26); use the stock-delegating launcher, or `--source-build` (unsupported) | Downloads the published, checksummed `aarch64-apple-darwin` runtime for your Codex version; compiles only with `--source-build` | No published runtime; `--source-build` only |

No platform silently starts a long local Codex build. The **Patched Codex Runtime** workflow publishes the Apple Silicon runtime automatically after each stable Codex release, usually within a few hours. Intel macOS patched runtimes were retired on 2026-09-26: their cold builds ran past 90 minutes on the Intel runner and no one outside the maintainer downloaded them. On an Intel Mac, `patch:codex` and `codex:sync` exit with that explanation and leave your `codex` untouched. `npm run patch:codex -- --make-default` downloads and verifies the runtime for your exact Codex version. If it is not published yet, the installer exits with the expected asset name and leaves the existing `codex` command untouched; local compilation happens only when you explicitly add `--source-build`.
