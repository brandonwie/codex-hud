...
> `5h` ve `7d` segmentleri, Codex rollout olaylarındaki uygun adaylar arasından en yeni hesap geneli limit anlık görüntüsünü kullanır. `codex` limit paketi ile limit kimliği olmayan eski anlık görüntüler uygundur; en yeni olay zaman damgası kazanır ve diğer adlı paketler yok sayılır. İki pencere aynı anlık görüntüden okunur ve uzunluklarına göre (300 ve 10080 dakika) eşleştirilir; bir pencere yoksa ilgili segment gösterilmez. Değerler yalnızca eşleşen bir rollout olayı bulunduğunda yenilenir.
...
# (runtime / "node vX" ve tkn / "Tkn:904k(...)" mevcuttur ancak varsayılan
# olarak kapalıdır — etkinleştirmek için ekleyin.)
...
bar = true          # false -> ctx/5h/7d çubuk grafiğini gizler, %'yi korur
barWidth = 5        # çubuk genişliği (hücre; 0 kapatır, en fazla 40)
barFilled = "█"     # çubuğun kullanılan kısmının glifi
barEmpty = "░"      # çubuğun kalan kısmının glifi
