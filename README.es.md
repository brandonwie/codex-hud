...
> Los segmentos `5h` y `7d` usan la instantánea válida más reciente de los límites de toda la cuenta en los eventos rollout de Codex. Son válidos el bucket `codex` y las instantáneas antiguas sin ID de límite; gana la marca de tiempo más reciente y se ignoran los demás buckets con nombre. Ambas ventanas se leen de la misma instantánea y se emparejan por duración (300 y 10080 minutos); si falta una ventana, se omite su segmento. Los valores solo se actualizan cuando hay un evento rollout coincidente.
...
# (runtime / "node vX" y tkn / "Tkn:904k(...)" están disponibles pero
# desactivados por defecto; agrégalos para activarlos.)
...
bar = true          # false -> oculta la barra de ctx/5h/7d, conserva el %
barWidth = 5        # ancho de la barra en celdas (0 la desactiva, máx. 40)
barFilled = "█"     # glifo de la parte usada de la barra
barEmpty = "░"      # glifo de la parte restante de la barra
