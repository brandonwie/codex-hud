...
> Сегменты `5h` и `7d` сопоставляются с окнами лимитов Codex по длине окна (300 и 10080 минут соответственно), а не по позиции в payload — бэкенд может передать любое окно в любом слоте или только одно из них. Сегмент, окно которого отсутствует в payload, показывает `?` (например, `5h:?`), а не заимствует значение другого окна.
