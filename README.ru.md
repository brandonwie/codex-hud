...
> Сегменты `5h` и `7d` используют самый новый подходящий общесистемный снимок лимитов аккаунта из событий rollout Codex. Подходят бакет `codex` и старые снимки без ID лимита; выбирается событие с самой новой временной меткой, а остальные именованные бакеты игнорируются. Оба окна берутся из одного снимка и сопоставляются по длительности (300 и 10080 минут); если окна нет, соответствующий сегмент опускается. Значения обновляются только при наличии подходящего события rollout.

Отрисовщик строки статуса по умолчанию — `codex-hud`, небольшой нативный бинарник на Rust (edition 2021, MIT): единый самодостаточный исполняемый файл без интерпретатора на пути отрисовки, с минимальным набором зависимостей (только `serde_json` и `toml`), полностью без `unsafe`-кода и с оптимизированной по размеру релизной сборкой около 574 KB. Для ясности: в этом README фигурируют два разных «Rust»: сам upstream-Codex CLI — это программа на Rust (цель сборки экспериментального патча ниже), тогда как `codex-hud` — это отдельный отрисовщик строки статуса в этом репозитории.
...
Самый быстрый путь. Установка собирается из исходников, поэтому нужны `git`, `node` и тулчейн Rust (`cargo`) в вашем `PATH`, а также рабочий `codex`:

```bash
curl -fsSL https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh | bash
```

Это клонирует закреплённый релиз, собирает отрисовщик `codex-hud`, устанавливает лаунчер с делегированием стоковому Codex и регистрирует плагин — он **не** трогает вашу существующую команду `codex`. Чтобы `codex` также указывал на лаунчер HUD, передайте флаг shell-у, который запускает скрипт: `curl -fsSL https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh | CODEX_HUD_MAKE_DEFAULT=1 bash`. Чтобы выполнить предпросмотр без каких-либо изменений, запустите `bash install.sh --dry-run` (или задайте `CODEX_HUD_DRY_RUN=1`). Чтобы проверить скрипт перед запуском, скачайте его командой `curl -fsSLO https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh`, прочитайте его, затем выполните `bash install.sh`.

Нет тулчейна Rust или предпочитаете ручной контроль? Используйте пошаговую установку ниже.
...
# (runtime / "node vX" и tkn / "Tkn:904k(...)" доступны, но по умолчанию
# выключены — добавьте их, чтобы включить.)
...
bar = true          # false -> скрыть шкалу ctx/5h/7d, оставить %
barWidth = 5        # ширина шкалы в ячейках (0 отключает, макс. 40)
barFilled = "█"     # символ заполненной части шкалы
barEmpty = "░"      # символ оставшейся части шкалы
...
pacePrefix = true   # false -> скрыть значок темпа (🐢/👾/🔥), оставить %
...
Ненулевой код возврата — только когда активная цепочка запуска сломана; деградация отрисовщика никогда не меняет статус healthy. Рекомендация о пересборке отрисовщика действует с точностью до релиза: версия `codex-hud`, зашитая при компиляции, сравнивается с `package.json`, поэтому рекомендация срабатывает, когда релиз сдвигает версию, а не на каждом коммите. Одна проверка конфигурации считается поломкой: лаунчер в режиме stock, когда `$CODEX_HOME/config.toml` объявляет `[tui].status_line_command`, — стоковый Codex игнорирует этот ключ, и футер молча исчезает (`npm run patch:codex` восстанавливает его, либо удалите ключ).
...
- Выпускать предсобранные релизные бинарники `codex-hud` с контрольными суммами SHA-256, чтобы установщик одной командой мог пропустить локальную сборку Rust.

## Участие в разработке

Issue и pull request приветствуются. См. [CONTRIBUTING.md](CONTRIBUTING.md) — там описаны рабочий процесс участия и полный справочник по скриптам сопровождающего.
