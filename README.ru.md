...
**HUD рабочего пространства для OpenAI Codex CLI — автономный многострочный HUD по умолчанию, плюс компактная цветная строка статуса (модель, проект, git, контекст, расход за 5ч/7д), которая в экспериментальном пропатченном режиме становится футером внутри TUI.**
...
Отрисовщик строки статуса по умолчанию — `codex-hud-rs`, небольшой Rust-бинарник из этого репозитория; Node-скрипт (`plugins/codex-hud/scripts/codex-hud.js`) остаётся документированным фолбэком и эталоном паритета. В этом README встречаются два разных «Rust»: сам upstream Codex CLI — это программа на Rust (цель сборки экспериментального патча ниже), а `codex-hud-rs` — отрисовщик строки статуса из этого репозитория.
...
Опционально соберите Rust-отрисовщик строки статуса. В TUI, запущенном в стоковом режиме, это ничего видимого не меняет — сборка важна для экспериментального пропатченного футера и для автономного использования `codex-hud-rs` (`--watch` или ручное подключение `status_line_command` к другим инструментам). После сборки установщик автоматически подхватывает отрисовщик (`--renderer auto`) для пропатченного режима и `--print-config`:

```bash
npm run build:rust   # опционально: собирает rust/target/release/codex-hud-rs
```
...
Запускайте Node-отрисовщик напрямую во время разработки:
...
Запустите локально `node plugins/codex-hud/scripts/codex-hud.js --line --color`, чтобы увидеть ту же строку со стилями ANSI-цветов.

`codex-hud-rs` (после `npm run build:rust`) предоставляет идентичный набор флагов — `--line` / `--status-line` / `--color` / `--json` / `--watch` / `--init-config` / `--print-config` / `--config-path`:
...
И многострочный HUD, и компактная строка статуса настраиваются через необязательный файл `codex-hud.toml`; Node- и Rust-отрисовщики читают одни и те же уровни `codex-hud.toml`. Без файла конфигурации вы получаете значения по умолчанию, показанные выше; каждый ключ является необязательным, и всё, что вы опускаете, наследует встроенное значение по умолчанию.
...
Ненулевой код возврата — только когда активная цепочка запуска сломана; деградация отрисовщика никогда не меняет статус healthy. Рекомендация о пересборке отрисовщика действует с точностью до релиза: версия `codex-hud-rs`, зашитая при компиляции, сравнивается с `package.json`, поэтому рекомендация срабатывает, когда релиз сдвигает версию, а не на каждом коммите.
...
Установщик патчит соответствующий тег OpenAI Codex, собирает Rust CLI и складывает исполняемый файл в стейджинг `~/.local/bin/codex-hud-codex.d/<version>/codex`. Стейджинговый пэйлоад обязан пройти health-чек `--version` **до** любой активации; только после этого `~/.local/bin/codex-hud-codex` атомарно перенаправляется на новый пэйлоад, а предыдущая версия остаётся на диске для отката. Неудачная сборка откладывается как `<version>.failed`, активный рантайм не трогается. Также записывается `~/.local/bin/codex-hud-tui` в пропатченном режиме — лаунчер, передающий цветную команду HUD через переопределение Codex `-c tui.status_line_command=...` без изменения `~/.codex/config.toml`. С `--renderer auto` (по умолчанию) внедряется команда `'~/.local/bin/codex-hud-rs' --line --color`, если Rust-отрисовщик установлен, и `node .../plugins/codex-hud/scripts/codex-hud.js --line --color` в качестве фолбэка в противном случае. Путь исполняемого файла и `argv[0]` сохраняют видимые как Codex имена, поэтому терминальные интеграции вроде Herdr по-прежнему распознают панель как сессию Codex.
...
# Фолбэк на Node (без сборки Rust):
...
# Фолбэк на Node — замените /path/to/codex-hud путём к вашему локальному клону:
...
├─ rust/                             # исходники codex-hud-rs (отрисовщик строки статуса по умолчанию)
└─ scripts/
   ├─ test-codex-hud.js
...
- Поддерживать `codex-hud-rs` в golden-паритете с Node-отрисовщиком (`npm run test:rust` проверяет оба отрисовщика на одних и тех же фикстурах).
