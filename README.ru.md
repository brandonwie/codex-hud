...
Основные команды сопровождения: `npm test`, `npm run test:rust`, `npm run check:i18n`, `npm run doctor`, `npm run sync:version` и `npm run measure:rust` (сообщает размер релизного бинарника (~574 KB) и сравнивает задержку Rust-отрисовщика со старым Node-отрисовщиком).
