...
よく使うメンテナーコマンド: `npm test`, `npm run test:rust`, `npm run check:i18n`, `npm run doctor`, `npm run sync:version`, `npm run measure:rust`（リリースバイナリのサイズ（およそ 574 KB）を報告し、Rust レンダラーのレイテンシを旧来の Node レンダラーと比較します）。
