...
> `5h` と `7d` のセグメントは、Codex の rollout イベントにある対象候補のうち、最新のアカウント全体レート制限スナップショットを使用します。対象は `codex` リミットバケットとリミット ID のない旧形式のスナップショットで、イベント時刻が最も新しいものを選び、その他の名前付きバケットは無視します。両方のウィンドウは同じスナップショットから読み取り、長さ（300 分と 10080 分）で対応付けます。ウィンドウがない場合、そのセグメントは省略されます。値は一致する rollout イベントが得られたときにのみ更新されます。
...
bar = true          # false -> ctx/5h/7d のバーグラフを隠し、% は残す
barWidth = 5        # バーグラフの幅（セル、0 で無効、最大 40）
barFilled = "█"     # バーグラフの使用済み部分のグリフ
barEmpty = "░"      # バーグラフの残り部分のグリフ
...
アクティブなエントリポイントチェーンが壊れている場合にのみ非ゼロで終了します — レンダラーの劣化が healthy ステータスを覆すことはありません。レンダラーの再ビルド推奨はリリース粒度です。コンパイル時に埋め込まれた `codex-hud` のバージョンを `package.json` と比較するため、リリースでバージョンが上がったときに発火し、コミットごとには発火しません。 設定チェックが 1 つだけ「壊れている」扱いになります。`$CODEX_HOME/config.toml` が `[tui].status_line_command` を宣言しているのにランチャーが stock モードの場合です。stock の Codex はこのキーを無視し、フッターは何も言わずに消えます（`npm run patch:codex` で復元するか、キーを削除してください）。
