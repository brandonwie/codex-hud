...
> `5h`와 `7d` 세그먼트는 페이로드 내 위치가 아니라 윈도우 길이(각각 300분, 10080분)로 Codex의 사용량 제한 윈도우와 매칭됩니다 — 백엔드는 어느 윈도우든 어느 슬롯으로 보고할 수 있고, 하나만 보고할 수도 있습니다. 페이로드에 해당 윈도우가 없는 세그먼트는 다른 윈도우의 값을 빌려 쓰지 않고 `?`를 표시합니다(예: `5h:?`).
