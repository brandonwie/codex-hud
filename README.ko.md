...
- 지원되는 커스텀 렌더러가 도입되면 패치를 폐기할 수 있도록 업스트림 Codex 상태 라인 변경 사항을 추적합니다.
- Codex 플러그인 디렉터리 카드가 게시 준비되면 스크린샷을 추가합니다.

## 기여하기

이슈와 풀 리퀘스트를 환영합니다. HUD 출력을 변경한 후에는 `npm test`와 Codex 플러그인 검증기를 실행하세요. 매니페스트 버전을 변경한 후에는 `codex plugin add codex-hud@codex-hud`로 로컬 플러그인을 재설치하세요.
