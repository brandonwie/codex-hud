...
기본 상태 라인 렌더러는 이 저장소의 작은 Rust 바이너리인 `codex-hud-rs`입니다. Node 스크립트(`plugins/codex-hud/scripts/codex-hud.js`)는 문서화된 폴백이자 패리티 검증 기준(parity oracle)으로 계속 유지됩니다. 이 README에는 서로 다른 두 가지 "Rust"가 등장합니다. 업스트림 Codex CLI 자체가 Rust 프로그램이고(아래 실험적 패치의 빌드 대상), `codex-hud-rs`는 이 저장소에 포함된 상태 라인 렌더러입니다.
...
선택적으로 Rust 상태 라인 렌더러를 빌드할 수 있습니다. 스톡으로 실행한 TUI에서는 눈에 보이는 변화가 전혀 없습니다 — 실험적인 패치 푸터를 쓰거나 `codex-hud-rs`를 독립 실행형으로 사용할 때(`--watch`, 또는 다른 도구에 `status_line_command`를 직접 연결할 때) 의미가 있습니다. 한 번 빌드해 두면 패치 모드와 `--print-config`에서 설치기가 자동으로 인식합니다(`--renderer auto`):

```bash
npm run build:rust   # 선택: rust/target/release/codex-hud-rs 빌드
```
...
`codex-hud-rs`(`npm run build:rust` 이후)는 동일한 플래그 집합을 제공합니다 — `--line` / `--status-line` / `--color` / `--json` / `--watch` / `--init-config` / `--print-config` / `--config-path`:
...
# Node 폴백 — /path/to/codex-hud를 로컬 클론 경로로 교체하세요:
...
├─ rust/                             # codex-hud-rs 소스 (기본 상태 라인 렌더러)
└─ scripts/
   ├─ test-codex-hud.js
...
- `codex-hud-rs`를 Node 렌더러와 골든 패리티(golden parity) 상태로 유지합니다(`npm run test:rust`가 두 렌더러를 같은 픽스처로 검증합니다).
