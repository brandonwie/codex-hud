...
> Các segment `5h` và `7d` dùng snapshot giới hạn toàn tài khoản mới nhất trong số các ứng viên hợp lệ ở sự kiện rollout của Codex. Bucket `codex` và snapshot cũ không có ID giới hạn đều hợp lệ; timestamp sự kiện mới nhất được chọn, còn các bucket có tên khác bị bỏ qua. Cả hai cửa sổ được đọc từ cùng một snapshot và khớp theo độ dài (300 và 10080 phút); nếu thiếu cửa sổ nào thì segment tương ứng sẽ bị lược bỏ. Giá trị chỉ được cập nhật khi có sự kiện rollout phù hợp.
...
# (runtime / "node vX" và tkn / "Tkn:904k(...)" có sẵn nhưng tắt theo mặc
# định — thêm vào để bật.)
...
bar = true          # false -> ẩn biểu đồ thanh ctx/5h/7d, giữ lại %
barWidth = 5        # độ rộng biểu đồ thanh theo ô (0 để tắt, tối đa 40)
barFilled = "█"     # ký tự phần đã dùng của biểu đồ thanh
barEmpty = "░"      # ký tự phần còn lại của biểu đồ thanh
