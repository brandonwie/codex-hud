...
> Os segmentos `5h` e `7d` usam o snapshot elegível mais recente dos limites da conta inteira nos eventos de rollout do Codex. São elegíveis o bucket `codex` e os snapshots legados sem ID de limite; vence o timestamp mais recente, e os demais buckets nomeados são ignorados. As duas janelas vêm do mesmo snapshot e são associadas pela duração (300 e 10080 minutos); se uma janela estiver ausente, o segmento correspondente é omitido. Os valores só são atualizados quando há um evento de rollout correspondente.
...
O caminho mais rápido. Ele compila a partir do código-fonte, então precisa de `git`, `node` e uma toolchain Rust (`cargo`) no seu `PATH`, além de um `codex` funcional:

```bash
curl -fsSL https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh | bash
```

Isso clona uma release fixada, compila o renderizador `codex-hud`, instala o launcher com delegação ao Codex original e registra o plugin — ele **não** mexe no seu comando `codex` existente. Para também fazer o `codex` resolver para o launcher do HUD, passe a flag para o shell que executa o script: `curl -fsSL https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh | CODEX_HUD_MAKE_DEFAULT=1 bash`. Para visualizar sem alterar nada, execute `bash install.sh --dry-run` (ou defina `CODEX_HUD_DRY_RUN=1`). Para auditar antes de executar, baixe com `curl -fsSLO https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh`, leia-o e então execute `bash install.sh`.

Sem toolchain Rust, ou prefere controle manual? Use a configuração passo a passo abaixo.
...
Opcionalmente, compile o renderizador da linha de status em Rust. Isso não muda nada visível em uma TUI iniciada com o Codex original — importa para o rodapé experimental com patch e para o uso independente do `codex-hud` (`--watch`, ou conectar manualmente o `status_line_command` em outras ferramentas). Uma vez compilado, o instalador o detecta automaticamente (`--renderer auto`) para o modo com patch e para `--print-config`:
...
```bash
npm run build:rust
./rust/target/release/codex-hud           # visão multilinha independente
./rust/target/release/codex-hud --line     # linha compacta única
./rust/target/release/codex-hud --line --color
./rust/target/release/codex-hud --json      # legível por máquina
./rust/target/release/codex-hud --watch 5   # atualiza a cada 5s
npm test
```
...
O `codex-hud` (após `npm run build:rust`) expõe a mesma superfície de flags — `--line` / `--status-line` / `--color` / `--json` / `--watch` / `--init-config` / `--print-config` / `--config-path`:
...
# (runtime / "node vX" e tkn / "Tkn:904k(...)" estão disponíveis, mas
# desativados por padrão — adicione-os para optar por incluí-los.)
...
bar = true          # false -> oculta a barra de ctx/5h/7d, mantém o %
barWidth = 5        # largura da barra em células (0 desativa, máx. 40)
barFilled = "█"     # glifo da parte usada da barra
barEmpty = "░"      # glifo da parte restante da barra
...
Os marcadores de ritmo comparam o uso com a taxa de consumo uniforme: slow fica mais de `thresholds.pace.crit` atrás do ritmo, fast fica mais de `thresholds.pace.crit` à frente, e a faixa intermediária é normal. Execute `codex-hud --print-config` para ver o conjunto completo de opções resolvidas.
...
├─ rust/                             # fonte do codex-hud (renderizador padrão da linha de status)
└─ scripts/
   ├─ test-codex-hud.js
...
- Distribuir binários de release pré-compilados do `codex-hud` com checksums SHA-256, para que o instalador de uma linha possa pular a compilação Rust local.

## Contribuindo

Issues e pull requests são bem-vindos. Consulte [CONTRIBUTING.md](CONTRIBUTING.md) para o fluxo de contribuição e a referência completa de scripts de mantenedor.
