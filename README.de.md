...
> Die Segmente `5h` und `7d` verwenden den neuesten geeigneten kontoweiten Rate-Limit-Snapshot aus Codex-Rollout-Ereignissen. Geeignet sind der Limit-Bucket `codex` und ältere Snapshots ohne Limit-ID; der neueste Ereignis-Zeitstempel gewinnt, andere benannte Buckets werden ignoriert. Beide Fenster werden aus demselben Snapshot gelesen und anhand ihrer Länge (300 bzw. 10080 Minuten) zugeordnet; fehlt ein Fenster, wird sein Segment weggelassen. Werte werden nur aktualisiert, wenn ein passendes Rollout-Ereignis vorliegt.
...
# (runtime / "node vX" und tkn / "Tkn:904k(...)" sind verfügbar, aber
# standardmäßig aus — zum Aktivieren hinzufügen.)
...
bar = true          # false -> Balkendiagramm für ctx/5h/7d ausblenden, % behalten
barWidth = 5        # Balkenbreite in Zellen (0 deaktiviert, max. 40)
barFilled = "█"     # Glyph für den genutzten Teil des Balkens
barEmpty = "░"      # Glyph für den restlichen Teil des Balkens
