...
> เซกเมนต์ `5h` และ `7d` ใช้ snapshot ขีดจำกัดระดับบัญชีที่เข้าเกณฑ์และใหม่ที่สุดใน rollout event ของ Codex โดยผู้สมัครที่เข้าเกณฑ์คือบัคเก็ต `codex` และ snapshot แบบเดิมที่ไม่มี limit ID ระบบจะเลือก event timestamp ที่ใหม่ที่สุดและไม่สนใจบัคเก็ตชื่ออื่น หน้าต่างทั้งสองมาจาก snapshot เดียวกันและจับคู่ตามความยาว (300 และ 10080 นาที) หากไม่มีหน้าต่างใด เซกเมนต์นั้นจะถูกละไว้ ค่าจะอัปเดตเมื่อมี rollout event ที่ตรงกันเท่านั้น

ตัวเรนเดอร์ status line ค่าเริ่มต้นคือ `codex-hud` ซึ่งเป็นไบนารี Rust แบบเนทีฟขนาดเล็ก (edition 2021, MIT) เป็นไฟล์ปฏิบัติการเดี่ยวที่สมบูรณ์ในตัว ไม่มีอินเทอร์พรีเตอร์อยู่บนเส้นทางการเรนเดอร์ มีรอยเท้าของ dependency น้อยที่สุด (มีแค่ `serde_json` และ `toml`) ไม่มีโค้ด `unsafe` เลย และเป็นบิลด์ release ที่ปรับให้เล็กที่สุดซึ่งมีขนาดราว 574 KB เพื่อความชัดเจน มี "Rust" สองตัวที่ต่างกันปรากฏใน README นี้: Codex CLI ต้นทางนั้นเป็นโปรแกรม Rust ในตัวมันเอง (เป็นเป้าหมายการบิลด์ของแพตช์แบบทดลองด้านล่าง) ส่วน `codex-hud` คือตัวเรนเดอร์ status line ที่อยู่ในรีโปนี้ซึ่งแยกต่างหาก
...
เส้นทางที่เร็วที่สุด มันบิลด์จากซอร์ส จึงต้องมี `git`, `node` และ Rust toolchain (`cargo`) อยู่บน `PATH` ของคุณ พร้อมทั้ง `codex` ที่ใช้งานได้:

```bash
curl -fsSL https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh | bash
```

คำสั่งนี้จะโคลน release ที่ปักหมุดไว้ บิลด์ตัวเรนเดอร์ `codex-hud` ติดตั้งตัวเรียกใช้แบบส่งต่อไปยัง Codex ดั้งเดิม และลงทะเบียนปลั๊กอิน — มัน**ไม่ได้**แตะต้องคำสั่ง `codex` ที่คุณมีอยู่ หากต้องการให้ `codex` ชี้ไปที่ตัวเรียกใช้ HUD ด้วย ให้ส่งแฟล็กไปยัง shell ที่รันสคริปต์: `curl -fsSL https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh | CODEX_HUD_MAKE_DEFAULT=1 bash` หากต้องการดูตัวอย่างก่อนโดยไม่เปลี่ยนแปลงอะไร ให้รัน `bash install.sh --dry-run` (หรือกำหนด `CODEX_HUD_DRY_RUN=1`) หากต้องการตรวจสอบก่อนรัน ให้ดาวน์โหลดด้วย `curl -fsSLO https://raw.githubusercontent.com/brandonwie/codex-hud/main/install.sh` อ่านมัน แล้วจึงรัน `bash install.sh`

ไม่มี Rust toolchain หรืออยากควบคุมด้วยตนเอง? ใช้การตั้งค่าแบบทีละขั้นด้านล่าง
...
อีกทางเลือกหนึ่ง คุณสามารถคอมไพล์ตัวเรนเดอร์ status line ที่เป็น Rust ได้ ขั้นตอนนี้ไม่เปลี่ยนแปลงสิ่งที่มองเห็นใน TUI ที่เรียกใช้แบบดั้งเดิมเลย — มันมีผลกับ footer แบบแพตช์ทดลองและการใช้ `codex-hud` แบบสแตนด์อโลน (`--watch` หรือการต่อ `status_line_command` เข้ากับเครื่องมืออื่นด้วยตนเอง) เมื่อคอมไพล์แล้ว ตัวติดตั้งจะหยิบไปใช้โดยอัตโนมัติ (`--renderer auto`) สำหรับโหมดแพตช์และ `--print-config`:

```bash
npm run build:rust   # ตัวเลือก: คอมไพล์ rust/target/release/codex-hud
```
...
```bash
npm run build:rust
./rust/target/release/codex-hud           # สแนปช็อตหลายบรรทัดแบบสแตนด์อโลน
./rust/target/release/codex-hud --line     # บรรทัดกะทัดรัดบรรทัดเดียว
./rust/target/release/codex-hud --line --color
./rust/target/release/codex-hud --json      # อ่านได้ด้วยเครื่อง
./rust/target/release/codex-hud --watch 5   # รีเฟรชทุก 5 วินาที
npm test
```
...
`codex-hud` (หลังจากรัน `npm run build:rust`) มีชุดแฟล็กเหมือนกันทุกประการ — `--line` / `--status-line` / `--color` / `--json` / `--watch` / `--init-config` / `--print-config` / `--config-path`:
...
# (runtime / "node vX" และ tkn / "Tkn:904k(...)" มีให้ใช้ได้แต่ปิดอยู่โดยค่าเริ่มต้น
# — เพิ่มเข้าไปเพื่อเปิดใช้งาน)
...
bar = true          # false -> ซ่อนแถบกราฟ ctx/5h/7d แต่คง % ไว้
barWidth = 5        # ความกว้างแถบกราฟเป็นเซลล์ (0 คือปิด สูงสุด 40)
barFilled = "█"     # อักขระส่วนที่ใช้ไปของแถบกราฟ
barEmpty = "░"      # อักขระส่วนที่เหลือของแถบกราฟ
...
จะออกด้วยรหัสที่ไม่ใช่ศูนย์ก็ต่อเมื่อห่วงโซ่จุดเข้าใช้งานที่ใช้อยู่เสียหายเท่านั้น — ความเสื่อมสภาพของตัวเรนเดอร์จะไม่มีวันพลิกสถานะที่ healthy คำแนะนำให้คอมไพล์ตัวเรนเดอร์ใหม่ทำงานที่ระดับ release: มันเปรียบเทียบเวอร์ชัน `codex-hud` ที่ฝังไว้ตอนคอมไพล์กับ `package.json` จึงจะแจ้งเมื่อ release ขยับเวอร์ชันเท่านั้น ไม่ใช่ทุกคอมมิต มีการตรวจสอบค่ากำหนดหนึ่งรายการที่นับว่าเสียหาย: ตัวเรียกใช้อยู่ในโหมด stock ขณะที่ `$CODEX_HOME/config.toml` ประกาศ `[tui].status_line_command` เพราะ Codex แบบ stock จะละเลยคีย์นี้และส่วนท้ายจะหายไปเงียบ ๆ (`npm run patch:codex` จะกู้คืนให้ หรือลบคีย์นั้นออก)
...
├─ rust/                             # ซอร์สของ codex-hud (ตัวเรนเดอร์ status line ค่าเริ่มต้น)
└─ scripts/
   ├─ test-codex-hud.js
...
- เผยแพร่ไบนารี release ของ `codex-hud` ที่บิลด์ไว้ล่วงหน้าพร้อม checksum แบบ SHA-256 เพื่อให้ตัวติดตั้งบรรทัดเดียวข้ามการบิลด์ Rust ในเครื่องได้

## การร่วมพัฒนา

ยินดีต้อนรับ issue และ pull request ดู [CONTRIBUTING.md](CONTRIBUTING.md) สำหรับขั้นตอนการร่วมพัฒนาและข้อมูลอ้างอิงสคริปต์สำหรับผู้ดูแลฉบับเต็ม
